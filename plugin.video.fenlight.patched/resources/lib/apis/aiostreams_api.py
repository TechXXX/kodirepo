...
default_base_url = 'https://aiostreamsfortheweebsstable.midnightignite.me'
timeout = 25.0
...
class AIOStreamsAPI:
	def __init__(self, manifest_url=None):
		...
		self.api_base = self._api_base_from_manifest(self.manifest_url)
		self.stream_base = self._stream_base_from_manifest(self.manifest_url)

	def enabled(self):
		return bool(self.api_base or self.stream_base)
	...
	def usenet_streams(self, media_type, imdb_id='', tmdb_id='', season=None, episode=None):
		try:
			...
			results = self._search_api_results(stremio_type, stremio_id)
	...
	def _api_base_from_manifest(self, manifest_url):
		manifest_url = (manifest_url or '').strip()
		if manifest_url in ('', 'empty_setting'): return ''
		if manifest_url.lower() == 'midnight': manifest_url = default_base_url
	...
	def _search_api_results(self, stremio_type, stremio_id):
		try:
			if not self.api_base: return []
			api_key = get_setting('fenlight.tb.token', 'empty_setting')
			if api_key in ('', 'empty_setting', None): return []
			user_data = {
				'sortCriteria': {'global': []},
				'deduplicator': {'enabled': False},
				'formatter': {'id': 'torrentio'},
				'presets': [{
					'type': 'torbox-search', 'instanceId': 'fenlight', 'enabled': True,
					'options': {
						'timeout': int(timeout * 1000), 'name': 'TorBoxSearch',
						'sources': ['usenet'], 'mediaTypes': [], 'useMultipleInstances': False,
						'userSearchEngines': True, 'onlyShowUserSearchResults': False
					}
				}],
				'services': [{'id': 'torbox', 'enabled': True, 'credentials': {'apiKey': api_key}}]
			}
			headers = {
				'User-Agent': user_agent, 'Accept': 'application/json',
				'x-aiostreams-user-data': base64.b64encode(json.dumps(user_data, separators=(',', ':')).encode()).decode()
			}
			...
			return [i for i in (self._parse_search_result(item) for item in results) if i]
		except Exception as e:
			logger('Fen Light Patched', 'AIOStreams Search API exception | %s' % str(e))
			return []

	def _parse_search_result(self, item):
		if item.get('cached') is not True: return None
		nzb = item.get('nzbUrl')
		if not nzb: return None
		file_name = item.get('filename') or item.get('folderName') or item.get('name') or ''
		if not file_name: return None
		size = self._to_int(item.get('size'))
		folder_size = self._to_int(item.get('folderSize'))
		return {
			'name': file_name, 'short_name': file_name, 'size': size or folder_size or 0,
			'nzb': nzb, 'hash': item.get('infoHash') or nzb,
			'direct_debrid_link': 'usenet_search', 'package': '',
			'package_size': folder_size if folder_size and size and folder_size > size else 0,
			'indexer': item.get('indexer'), 'age': item.get('age')
		}
	...
	def _parse_stream(self, stream):
		...
		return {
			'name': file_name, 'short_name': file_name, 'size': size or folder_size or 0,
			'nzb': nzb, 'hash': stream.get('infoHash') or nzb,
			'direct_debrid_link': 'usenet_search', 'package': '',
			'package_size': folder_size if folder_size and size and folder_size > size else 0,
