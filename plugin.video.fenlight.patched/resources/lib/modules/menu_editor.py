...
get_all_icon_vars, addon_info = kodi_utils.get_all_icon_vars, kodi_utils.addon_info
main_list_name_dict = {'RootList': 'Root', 'MovieList': 'Movies', 'TVShowList': 'TV Shows', 'AnimeList': 'Animes'}
plugin_prefix = 'plugin://%s/?' % addon_info('id')
default_path = build_url({'mode': 'navigator.main', 'full_list': 'true'})
...
class MenuEditor:
	...
	def _get_menu_item(self, path):
		return dict(parse_qsl(path.replace(plugin_prefix, '')))
