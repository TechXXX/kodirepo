...
destination_check = translate_path('special://home/addons/plugin.video.fenlight.patched/')
changelog_location = translate_path('special://home/addons/plugin.video.fenlight.patched/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.fenlight.patched'
zipfile_name = 'plugin.video.fenlight.patched-%s.zip'
...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.fenlight.patched' in i['name'] \
