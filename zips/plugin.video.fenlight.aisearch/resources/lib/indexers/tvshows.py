...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('fenlight.aisearch.exit_params', folderpath)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			set_properties({'fenlight.aisearch.extras_params': extras_params, 'fenlight.aisearch.options_params': options_params, 'fenlight.aisearch.more_like_this_params': more_like_this_params})
