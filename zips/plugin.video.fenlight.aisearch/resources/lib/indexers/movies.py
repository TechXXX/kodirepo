...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('fenlight.aisearch.exit_params', folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({'fenlight.aisearch.extras_params': extras_params, 'fenlight.aisearch.options_params': options_params,
							'belongs_to_collection': belongs_to_movieset, 'fenlight.aisearch.more_like_this_params': more_like_this_params})
	...
	def worker(self):
		...
		self.watched_title = 'Trakt' if self.watched_indicators == 1 else 'fenlight.aisearch'
