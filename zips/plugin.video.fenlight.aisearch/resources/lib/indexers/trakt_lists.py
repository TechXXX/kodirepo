	...
	try:
		...
		if shuffle:
			...
			if returning_to_list:
				try: lists = json.loads(get_property('fenlight.aisearch.trakt.lists.order'))
				except: pass
			else:
				random.shuffle(lists)
				set_property('fenlight.aisearch.trakt.lists.order', json.dumps(lists))
			sort_method = 'none'
		else:
			clear_property('fenlight.aisearch.trakt.lists.order')
			sort_method = 'label'
...
def build_trakt_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: set_property('fenlight.aisearch.exit_params', folder_path())
