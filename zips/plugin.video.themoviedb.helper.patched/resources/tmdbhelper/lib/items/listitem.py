...
class _Video(_ListItem):
    ...
    def context_menu_selectplayer(self):
        head = '$ADDON[plugin.video.themoviedb.helper.patched 32322]'
    ...
    def context_menu_choosedefault(self):
        head = '$ADDON[plugin.video.themoviedb.helper.patched 32476]'
        path = f'Runscript(plugin.video.themoviedb.helper.patched,{self.context_menu_choosedefault_paramstring})'
        return (head, path)
