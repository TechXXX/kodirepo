    ...
    class BedrockClient(TranslationClient):
        ...
        def _request_translation(self, prompt : TranslationPrompt, temperature : float = None) -> Translation:
            ...
            if not self.access_key:
                raise TranslationImpossibleError('Access key must be provided as an argument')

            if not self.secret_access_key:
                raise TranslationImpossibleError('Secret access key must be provided as an argument')

            if not self.aws_region:
                raise TranslationImpossibleError('AWS region must be provided as an argument')
