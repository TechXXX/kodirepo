...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	try: preselect = [fl.index(i) for i in get_setting('fenlight.aisearch.%s' % quality_setting).split(', ')]
	except: preselect = []
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('fenlight.aisearch.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('fenlight.aisearch.', ''), default_setting_values(setting_id.replace('fenlight.aisearch.', ''))['setting_default'])
					button_action = get_setting('fenlight.aisearch.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
	...
	try: preselect = [fl.index(i) for i in get_setting('fenlight.aisearch.%s' % filter_setting_id).split(', ')]
	except: preselect = []
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('fenlight.aisearch.%s' % setting), default_setting_values(setting)['setting_default']
