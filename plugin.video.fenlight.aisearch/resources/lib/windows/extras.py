...
setting_base, label_base, ratings_icon_base = 'fenlight.aisearch.extras.%s.button', 'button%s.label', 'fenlight_flags/ratings/%s'
separator = '[B]  •  [/B]'
	...
	def assign_buttons(self):
		...
		for item in button_ids[:-1]:
			...
			try:
				...
				self.restore_setting_default({'setting_id': setting_id.replace('fenlight.aisearch.', ''), 'silent': 'true'})
				button_action = self.get_setting(setting_id)
				button_label = extras_button_label_values[self.media_type][button_action]
