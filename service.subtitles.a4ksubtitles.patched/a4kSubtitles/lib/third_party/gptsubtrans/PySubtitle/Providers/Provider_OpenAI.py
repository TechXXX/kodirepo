...
class OpenAiProvider(TranslationProvider):
    ...
    def GetAvailableModels(self) -> list[str]:
        """
        Returns a list of possible values for the LLM model using MinimalOpenAIClient
        """
        try:
            if not self.api_key:
            ...
            # response is expected to be a dict with a 'data' key containing a list of models
            ...
            model_list = [model for model in model_list if not any(exclude in model for exclude in self.excluded_model_types)]

            # If no valid models found, return all available model ids
            if not model_list:
